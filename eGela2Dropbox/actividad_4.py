...
def delete_folder():
    popup, progress_var, progress_bar = helper.progress("delete_folder", "Deleting folder...")
    progress = 0
    progress_var.set(progress)
    progress_bar.update()
    progress_step = float(100.0 / len(selected_items2))
...
def share_file_email(email):

    popup, progress_var, progress_bar = helper.progress("share:file", "Sharing File...")
    progress = 0
    progress_var.set(progress)
    progress_bar.update()
    progress_step = float(100.0 / len(selected_items2))
    ...
    for each in selected_items2:
        ...
        dropbox.share_file_email(path, email)
...
def share_file():
    ...
    label = tk.Label(login_frame, text="Share file")
    label.pack(side=tk.TOP)
    entry_field = tk.Entry(login_frame, width=35)
    entry_field.bind("<Return>", share_file_email)
    entry_field.pack(side=tk.TOP)
    send_button = tk.Button(login_frame, text="Send", command=lambda: share_file_email(entry_field.get()))
...
def get_space():
    ...
    label = tk.Label(login_frame, text="Espacio Usado (mega)")
    label.pack(side=tk.TOP)
    megaTotal, megaUsed = dropbox.get_space()




    label2 = tk.Label(login_frame, text=str(megaUsed) + " mega / " + str(megaTotal) + " mega")
    label2.pack(side=tk.TOP)

    label2 = tk.Label(login_frame, text= "" + str((megaUsed/megaTotal)*100)[0:6] + "% usado")
    label2.pack(side=tk.TOP)
    dropbox._root = popup
...
print("Egela login : ", egela._login)
if not egela._login:
    exit()
...
button2 = tk.Button(frame2, borderwidth=4, background="red", text="Delete File", width=10, pady=8, command=delete_files)
button2.pack(padx=2, pady=2)
button5 = tk.Button(frame2, borderwidth=4, background="red", text="Delete Folder", width=10, pady=8, command=delete_folder)
...
button4 = tk.Button(frame2, borderwidth=4, text="Share File", width=10, pady=8, command=share_file)
button4.pack(padx=2, pady=2)
button5 = tk.Button(frame2, borderwidth=4, text="Get Space", width=10, pady=8, command=get_space)
button5.pack(padx=2, pady=2)
